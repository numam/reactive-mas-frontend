...
const NODE_COLORS = {
  ...
  retail:         "from-sage-400 to-sage-600",
};
  ...
  return (
    <div
      ...
      className={`bg-white dark:bg-gray-900 rounded-2xl border transition-all cursor-pointer p-5 hover:shadow-sm ${
        selected
          ? "border-sage-400 dark:border-sage-600 shadow-md"
          : "border-sage-100 dark:border-gray-800"
      }`}
    >
      ...
      {agent.loading ? (
        <div className="space-y-1.5">
          <div className="h-3 bg-sage-100 dark:bg-gray-800 rounded animate-pulse w-3/4" />
          <div className="h-3 bg-sage-100 dark:bg-gray-800 rounded animate-pulse w-1/2" />
        </div>
      ) : (
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-sage-100 dark:border-gray-800">
        <div>
        ...
        <button className="text-xs text-sage-600 dark:text-sage-400 hover:text-sage-800 dark:hover:text-sage-300 font-medium border border-sage-200 dark:border-sage-800 hover:border-sage-400 dark:hover:border-sage-600 px-3 py-1.5 rounded-lg transition-colors">
...
function NodeDetail({ nodeType }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-sage-100 dark:border-gray-800 shadow-xs p-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <div>
          ...
          <div className="space-y-2">
            {Object.entries(variables).map(([k, v]) => (
              <div key={k} className="flex items-start justify-between py-2 border-b border-sage-100 dark:border-gray-800 last:border-0">
                <div>
                  <p className="text-sm font-semibold font-mono text-sage-700 dark:text-sage-400">{k}</p>
        ...
        <div>
          ...
          <div className="space-y-2">
            <p className="text-sm font-mono font-semibold text-sage-700 dark:text-sage-400">Disruption</p>
            {Object.entries(thresholds.disruption ?? {}).map(([k, v]) => (
              <div key={k} className="flex items-center justify-between py-2 border-b border-sage-100 dark:border-gray-800 last:border-0">
            ...
            <p className="text-sm font-mono font-semibold text-sage-700 dark:text-sage-400 pt-2">Recovery</p>
